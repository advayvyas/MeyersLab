...
# Explicitly replicate b1 and b2 for broadcasting
h <- tanh(X %*% W1 + b1[rep(1, n_obs), ])
mu <- h %*% W2 + b2[rep(1, n_obs), ]
...
posterior_means <- calculate(mu, values = draws)[[1]] # 1000 × 173

# Compute per-observation mean
pred_mean <- apply(posterior_means, 2, mean)      # length 173
# Compute 95% credible intervals per observation
pred_ci <- apply(posterior_means, 2, quantile, c(0.025, 0.975)) # 2 × 173
pred_ci <- t(pred_ci)  # transpose to 173 × 2
...
```{r bnn_plot, fig.width = 10, fig.height=8}
