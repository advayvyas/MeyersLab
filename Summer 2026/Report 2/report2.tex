...
Would like to show how different state vs local difference is, RMSE is useful for finding peak magnitude, find the differences in peak height and timing, explained well. Looking at areas or certain places from past seasons or weeks to predict the current week or season's values. HSA values are the ``stand-in'' for the ``forecast'' in state values. Best method to find peak diff / peak timing.

Needs confidence intervals, probability of being ahead / behind, 95 percent CI of probability would be useful. Estimate the quantile regression, when the data does not follow a distribution and can do this way instead. 

We have in-built shape guarantees from the nature of influenza, always steep/sharp parabolas.
