...
This week, I will further investigate Frechet distance and dynamic time warping and try to develop an intuition or way to discern differences between state and local forecasting levels through an examination of the plots of these two metrics. Also, I'll investigate Shannon entropy. Lastly, I'll take a look through a few forecasting models from FluSight (the dashboard) and then try to find some good models.
...
From the path variable in res_dtw, we can create and understand several timing distortion statistics. The first one is $\Delta t$, which is the local time shift for each time index. Basically, what time steps in HSA are matched to which time step in state. Therefore, positive means state lags, and negative means state leads.
...
dtw_stats = function(hsa_id = 688) {
  ...
  stats_list = list(
    ...
    amp_ratio    = median(P[i,1] / Q[j,1], na.rm=TRUE),
...
```
## Metric explanations
* Mean absolute lag is the average absolute time shift between matched points (points in P are aligned with points in Q about this many steps away)
* Mean lag but this time it is signed and that means it is usually smaller and portrays more accurately what an average time step would do
* Standard deviation of the time lag
* Mean absolute error along the DTW-warped path (how well shapes match)
* Root mean squared error along the DTW-warped path (penalizes larger differences more than the MAE)
* Root mean squared error along the regular path (for comparison)
* Median ratio of amplitudes along DTW-warped path (how much percent amplitude HSA is of state)
* Correlation between absolute lag and absolute amplitude difference along DTW-warped path (negative means large lag small amplitude together, positive is opposite), sees if these variables are related for this HSA ID

# Conclusion
Shannon entropy didn't really have anything interesting related to these models and this sort of question so I couldn't really find anything. I didn't have time to get around to FluSight models this week because I was messing around with DTW but I'll check it out next week.

DTW looks really good here and these metrics seem to be a good solution for the questions you had earlier regarding comparing these two curves.
